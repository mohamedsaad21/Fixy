...
public class PaymentsController : AppControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [HttpGet(Router.PaymentRouting.GetTechnicianCashCommissionsOwed)]
    public async Task<IActionResult> GetTechnicianCashCommissionsOwed()
    {
        return ToActionResult(await Mediator.Send(new GetTechnicianCashCommissionsOwedQuery()));
    }
}
